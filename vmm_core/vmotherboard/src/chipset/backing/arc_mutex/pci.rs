...
impl BusResolverWeakMutexPcie {
    pub fn resolve(mut self) -> Result<(), Vec<PcieConflict>> {
        ...
        tracing::info!("dumping enumerators ---------------------------------------------");
        for (k, _) in &self.enumerators {
            tracing::info!("{:?}", k);
        }

        tracing::info!("dumping ports ---------------------------------------------------");
        for (k, (p, e)) in &self.ports {
            tracing::info!("{:?} -> {} -> {:?}", k, p, e);
        }

        tracing::info!("dumping devices -------------------------------------------------");
        for WeakMutexPcieDeviceEntry{ bus_id_port, name, dev: _ } in &self.devices {
            tracing::info!("{:?}: {}", bus_id_port, name);
        }

        for WeakMutexPcieDeviceEntry{ bus_id_port, name, dev } in self.devices {
