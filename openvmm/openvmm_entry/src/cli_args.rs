...
impl FromStr for PcieRootComplexCli {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let mut high_mmio = 1 * 1024 * 1024 * 1024;
        for opt in opts {
            let mut s = opt.split('=');
            ...
            match opt {
                ...
                "low_mmio" => {
                    ...
                    low_mmio = parse_memory(low_mmio_str).context("failed to parse low MMIO size")?.try_into()?;
                },
                "high_mmio" => {
                    let high_mmio_str = s.next().context("expected high MMIO size")?;
                    high_mmio = parse_memory(high_mmio_str).context("failed to parse high MMIO size")?;
                },
                opt => anyhow::bail!("unknown option: '{opt}'"),
            }
        }
