        ...
        for opt in opts {
            ...
            match opt {
                ...
                "start_bus" => {
                    ...
                    start_bus = u8::from_str(bus_str).context("failed to parse start bus number")?;
                },
                "end_bus" => {
...
mod tests {
    ...
    fn test_pcie_root_complex_from_str() {
        assert_eq!(
            ...
            PcieRootComplexCli { name: "rc0".to_string(), segment: 0, start_bus: 0, end_bus: 255 });

        assert_eq!(
            PcieRootComplexCli::from_str("rc1,segment=1").unwrap(),
            PcieRootComplexCli { name: "rc1".to_string(), segment: 1, start_bus: 0, end_bus: 255 });

        assert_eq!(
            PcieRootComplexCli::from_str("rc2,start_bus=32").unwrap(),
            PcieRootComplexCli { name: "rc2".to_string(), segment: 0, start_bus: 32, end_bus: 255 });

        assert_eq!(
            PcieRootComplexCli::from_str("rc3,end_bus=31").unwrap(),
            PcieRootComplexCli { name: "rc3".to_string(), segment: 0, start_bus: 0, end_bus: 31 });

        assert_eq!(
            PcieRootComplexCli::from_str("rc4,start_bus=32,end_bus=127").unwrap(),
            PcieRootComplexCli { name: "rc4".to_string(), segment: 0, start_bus: 32, end_bus: 127 });

        assert_eq!(
            PcieRootComplexCli::from_str("rc5,segment=2,start_bus=32,end_bus=127").unwrap(),
            PcieRootComplexCli { name: "rc5".to_string(), segment: 2, start_bus: 32, end_bus: 127 });
    ...
    fn test_pcie_root_port_from_str() {
        assert_eq!(
            ...
            PcieRootPortCli { root_complex_name: "rc0".to_string(), name: "rc0rp0".to_string() });

        assert_eq!(
            PcieRootPortCli::from_str("my_rc:port2").unwrap(),
            PcieRootPortCli { root_complex_name: "my_rc".to_string(), name: "port2".to_string() });
