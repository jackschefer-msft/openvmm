...
pub struct GenericPcieRootComplex {
    ...
    ports: HashMap<u8, (Arc<str>, GenericPcieRootPort)>,
}
...
impl GenericPcieRootComplex {
    ...
    pub fn new(
        ...
        let port_map = ports.into_iter()
            .enumerate()
            .map(|(i, definition)| {
                let device_number = (i << 3) as u8;
                let emulator = GenericPcieRootPort::new();
                (device_number, (definition.name.into(), emulator))
            })
            .collect();
    ...
    pub fn downstream_ports(&self) -> Vec<(u8, Arc<str>)> {
        self.ports.iter()
            .map(|(port, (name, _))| (port.clone(), name.clone()))
            .collect()
    }
    ...
    fn decode_ecam_access(&self, addr: u64) -> (u8, u8, u16) {
        ...
        let bus_offset = ((bdf & 0xFF00) >> 8) as u8;
        let bus = bus_offset + self.start_bus;
...
impl MmioIntercept for GenericPcieRootComplex {
    fn mmio_read(&mut self, addr: u64, data: &mut [u8]) -> IoResult {
        ...
        if bus == self.start_bus {
            ...
            if let Some((_, port)) = self.ports.get_mut(&device_function) {
            ...
            for (_, (_, port)) in &mut self.ports {
                if port.assigned_bus_number(bus) {
                    let _ = port.forward_cfg_read(bus, device_function, cfg_offset & !3, &mut value);
                    break
                }
            }
        } else {
    ...
    fn mmio_write(&mut self, addr: u64, data: &[u8]) -> IoResult {
        ...
        let (bus, device_function, cfg_offset) = self.decode_ecam_access(addr);
        tracing::trace!("ecam write addr 0x{:016x} decodes to offset 0x{:04x} on {:02x}:{:02x}",
            addr, cfg_offset, bus, device_function);
            ...
            for (_, (_, port)) in &mut self.ports {
                if port.assigned_bus_number(bus) {
                    ...
                    let merged_value = if data.len() == 4 {
                        ...
                        let mut temp: u32 = 0;
                        let _ = port.forward_cfg_read(bus, device_function, rounded_offset, &mut temp);
                        combine_old_new_values(cfg_offset, temp, write_value, data.len())
                    };

                    let _ = port.forward_cfg_write(bus, device_function, rounded_offset, merged_value);
                }
            }
        } else {
            tracing::error!("unexpected intercept")
...
impl GenericPcieRootPort {
    ...
    fn forward_cfg_read(&mut self, bus: u8, device_function: u8, cfg_offset: u16, value: &mut u32) -> IoResult {
        ...
        if bus == secondary_bus_number && device_function == 0 {
            ...
            tracing::error!("multi-level hierarchies not implemented yet")
        }
