...
impl<T: RingMem> ProcessLoop<T> {
    ...
    pub(crate) async fn run(&mut self, mut recv: mesh::Receiver<Msg>) -> Result<(), FatalError> {
        ...
        loop {
            ...
            let event = {
                ...
                // of one another.
                let run_next_primary_host_request = async {
    ...
    {
        self.push_primary_host_request_handler(|mut access| {
    ...
    fn process_host_request(&mut self, message: Msg) -> Result<(), FatalError> {
        match message {
            ...
            Msg::PowerState(state) => {
                ...
                self.push_primary_host_request_handler(move |mut access| async move {
